...
$app->post('/api/setDownload', function(Request $request, Response $response){
    ...
    if($userId == ""){
        if(($first_name != "") && ($last_name != "") && ($email != "")){
            $sql = "INSERT INTO msp_person (first_name,last_name,email)
    ...
    // Set Download
    $sql = "INSERT INTO msp_download (user_id, institute, intended_use )
    ...
    try{
        ...
        $sql = "SELECT * FROM msp_download WHERE user_id = '$userId' ORDER BY date DESC";
        $stmt = $db->query($sql);
    ...
    foreach ($guideSelected as $guideId) {
        $sql = "INSERT INTO msp_download_guidelines (download_id, guideline_id)
    ...
    foreach ($instituteRegions as $regionId) {
        $sql = "INSERT INTO msp_download_regions (download_id, region_id, region_scope)
